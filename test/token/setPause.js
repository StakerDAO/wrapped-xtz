...
contract('TZIP-7 token contract', () => {
    ...
    beforeEach(async () => {
        ...
        tzip7Instance = await tzip7.new(_tzip7InitialStorage.setPause);
    ...
    describe("Pause", () => {
        ...
        describe('effects of setPause', () => {
            ...
            it('should fail for approve operations', async () => {
                const owner = bob;
                await _taquitoHelpers.setSigner(owner.sk);
            ...
            it('should fail for approveCAS operations', async () => {
                const owner = bob;
                const spender = carol;
                await _taquitoHelpers.setSigner(owner.sk);

                const currentAllowanceValue = await helpers.tzip7.getAllowance(owner.pkh, spender.pkh)
                const operationPromise = helpers.tzip7.approveCAS(
                    currentAllowanceValue, // expected allowance value
                    spender.pkh, // spender
                    10000 // allowance value
                );
                await expect(operationPromise).to.be.rejectedWith(contractErrors.tzip7.tokenOperationsPaused);
            });

            it('should fail for mint operations', async () => {
            ...
            it('should fail for burn operations', async () => {
                await _taquitoHelpers.setSigner(admin.sk);
