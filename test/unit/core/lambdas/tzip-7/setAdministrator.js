    ...
    describe('setAdministrator', () => {
        it('should not be callable by other address than the admin', async () => {
            const operationPromise = _taquitoHelpers.signAs(bob.sk, async () => {
                return helpers.core.setAdministrator(newAdministrator) 
            });
        ...
        describe('emitted operations', () => {
            ...
            let firstOperation;
            beforeEach(async () => {
                const operation = await helpers.core.setAdministrator(newAdministrator);
            ...
            it('should emit a setAdministrator operation on the tzip-7 token contract', async () => {
                expect(firstOperation.destination).to.be.equal(helpers.tzip7.instance.address);
                expect(firstOperation.parameters.entrypoint).to.be.equal('setAdministrator');
                /**
